-- Comprehensive RLS Policy Fix (Without Storage Policies)
-- This migration addresses infinite recursion and policy conflicts
-- Storage policies must be handled through Supabase dashboard
...
**For the storage policies**, you'll need to handle those separately through your Supabase dashboard:

1. Go to your Supabase dashboard
2. Navigate to **Storage** → **Policies**
3. Manually create these policies:

**Profile Pictures Bucket:**
```sql
-- Policy Name: "Profile pictures: Upload own files"
-- Operation: INSERT
-- Target roles: authenticated
-- USING expression: bucket_id = 'profile-pictures' AND auth.uid()::text = (storage.foldername(name))[1]

-- Policy Name: "Profile pictures: Public read access" 
-- Operation: SELECT
-- Target roles: authenticated, anon
-- USING expression: bucket_id = 'profile-pictures'
```

**Community Avatars Bucket:**
```sql
-- Policy Name: "Community avatars: Public read access"
-- Operation: SELECT  
-- Target roles: authenticated, anon
-- USING expression: bucket_id = 'community-avatars'

-- Policy Name: "Community avatars: Upload with proper structure"
-- Operation: INSERT
-- Target roles: authenticated
-- USING expression: bucket_id = 'community-avatars' AND auth.uid() IS NOT NULL AND (storage.foldername(name))[1] = 'communities'
```

This approach separates the table policies (which you can run via SQL) from the storage policies (which need to be handled through the dashboard due to permission restrictions).